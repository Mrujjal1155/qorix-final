CREATE OR REPLACE FUNCTION public.release_stock_notification(_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
END;
$$;
REVOKE ALL ON FUNCTION public.release_stock_notification(uuid) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.release_stock_notification(uuid) TO service_role;
...
BEGIN
  ...
  IF event_kind IS NOT NULL THEN
    SELECT coalesce(max(id)::text, 'initial') INTO cycle_id
      FROM public.stock_notification_events
     WHERE product_id = _product_id AND kind = 'restock';
